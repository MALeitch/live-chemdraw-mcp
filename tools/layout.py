...
def register(mcp, bridge):
    ...
    @mcp.tool(description=(
        ...
        "(points, 72/inch). object_id may be a structure OR a free-floating "
        "annotation — an arrow (chemdraw_make_arrow/chemdraw_list_arrows), "
        "a symbol/bracket/tlc_plate, or a caption including an unowned one "
        "(e.g. a reaction scheme's reagents_text/conditions_text) — any id "
        "chemdraw_get_document_state/chemdraw_list_arrows/etc. shows as "
        "present will resolve here. Every call snapshots the document "
        "before and after and reports `unexpected_moves` — any object that "
        "moved WITHOUT being in your list (e.g. a counterion or satellite "
        "fragment that turned out to be carried along by a different "
        "object's move); this check only covers STRUCTURES, not "
        "annotations — an annotation being unexpectedly dragged along by "
        "something else is not currently known to happen but would not be "
        "caught here. ALWAYS check `unexpected_moves` in the result; if "
        ...
        "`resulting_bounds` for every moved object (structure or "
        "annotation) — use those instead of a follow-up read to verify the "
        "layout. Also check `violations.off_page` — moving anything can "
        "push it outside the document's actual page, which the preview "
        "image (auto-cropped to whatever was drawn) will NOT reveal; this "
        "check covers moved annotations too, not just structures. An id "
        "that cannot be found or cannot be moved comes back in `missing` "
        "rather than raising. A .cdxml backup is saved first; the result "
